...
describe EventsController do
  ...
  let(:valid_attributes) {
    { scheduled: rand(7).day.since }
  }

  let(:invalid_attributes) { {'bad'=>'bad'} }
  let(:valid_session) { {} }
  ...
  describe "PUT update" do
    describe "with valid params" do
      let(:new_attributes) { { scheduled: rand(8..10).day.since } }

      xit "updates the requested event" do
        Event.any_instance.should_receive(:update).with(new_attributes)
        event = Event.create! valid_attributes
